...
# Stage2 must load the trained three branch-conditioned interleaved latent transition blocks from Stage1.
...

{
  ...
  echo "stage: residual_sid_stage2_branch_interleaved_latent3"
...
# residual + branch-conditioned interleaved latent-anchor weights must come from the converted Stage1 model.
torchrun \
  --nnodes=1 \
  --nproc_per_node=8 \
  --rdzv_backend=c10d \
