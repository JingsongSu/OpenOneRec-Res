...
def generate_itemic_tokens(itemic_layer_n: int, vocab_size_per_layer: int, last_layer_vocab_size: int, time_token_size: int, ctype_token_size: int, mid_vocab_size_per_layer: int, ls_vocab_size_per_layer: int) -> List[str]:
    ...
    #目前最后一层sid冲突数大于码本大小，所有最后一层单独处理
    for layer_name in layer_names[:-1]:
        for i in range(vocab_size_per_layer):
            new_tokens.append(f"<s_{layer_name}_{i}>")
    
    #处理最后一层
    for i in range(last_layer_vocab_size):
        new_tokens.append(f"<s_{layer_names[-1]}_{i}>")
    ...
    for i in range(time_token_size):
        new_tokens.append(f'<|time_{i}|>')
    
    
    #添加ctype token
    for i in range(ctype_token_size):
        new_tokens.append(f'<|ctype_{i}|>')
    
    #添加mid的sid token
    for layer_name in layer_names[:3]:
        for i in range(mid_vocab_size_per_layer):
            new_tokens.append(f"<mid_{layer_name}_{i}>")

    for layer_name in layer_names[:2]:
        for i in range(ls_vocab_size_per_layer):
            new_tokens.append(f"<ls_{layer_name}_{i}>")
    
    
    #更改total_tokens计算方式
    total_tokens = (itemic_layer_n - 1) * vocab_size_per_layer + last_layer_vocab_size  + 2 + time_token_size + ctype_token_size + 3 * mid_vocab_size_per_layer + 2 * ls_vocab_size_per_layer
...
def main():
    ...
    parser.add_argument(
        "--last_layer_vocab_size",
        type=int,
        required=True,
        help="the last layer avoid sid collision"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="the ctype token size"
    )
    
    parser.add_argument(
        "--mid_vocab_size_per_layer",
        type=int,
        required=True,
        help="Vocabulary size per layer (e.g., 8192 for tokens from 0 to 8191)"
    )
    ...
    try:
        ...
        new_tokens = generate_itemic_tokens(
            ...
            last_layer_vocab_size=args.last_layer_vocab_size,
            time_token_size=args.time_token_size,
            ctype_token_size=args.ctype_token_size,
            mid_vocab_size_per_layer=args.mid_vocab_size_per_layer,
            ls_vocab_size_per_layer=args.ls_vocab_size_per_layer
        )
