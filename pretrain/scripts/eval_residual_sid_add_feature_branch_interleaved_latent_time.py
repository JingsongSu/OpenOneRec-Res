...
with historical time tokens.

Raw target data and residual-SID prompt suffix stay exactly in the original form:

    <target_ctype><|sid_begin|>

Historical user behavior is allowed/required to contain one atomic time token
after every historical SID:

    <history_ctype><|sid_begin|><s_a_*><s_b_*><s_c_*><s_d_*><|sid_end|><|time_*|>
...
DATA_PATH = os.environ.get(
    ...
    (
        "/home/jovyan/ceph-1/sujinsong/sujinsong/OpenOneRec-latent/"
        "output/eval/sft_video_rec_add_feature_time.parquet"
    ),
)
...
HISTORY_EVENT_WITH_TIME_RE = re.compile(
    r"<(?:\|)?ctype_\d+(?:\|)?>"
    r"<\|sid_begin\|>"
    r"<s_a_\d+>"
    r"<s_b_\d+>"
...
def validate_sample(
    ...
    # New time-token format:
    # every historical SID must be immediately followed by exactly one <|time_x|>.
    time_tokens = TIME_RE.findall(input_text)

    if not time_tokens:
        raise ValueError(
            f"{row_context}: input side contains no TIME token"
        )

    if TIME_RE.search(answer_text):
        raise ValueError(
            f"{row_context}: assistant target must not contain TIME token"
        )

    history_sid_begin_count = input_text.count(SID_BEGIN)
    history_sid_end_count = input_text.count(SID_END)

    if history_sid_begin_count != history_sid_end_count:
        raise ValueError(
            f"{row_context}: historical SID_BEGIN/SID_END count mismatch; "
            f"sid_begin={history_sid_begin_count}, "
            f"sid_end={history_sid_end_count}"
        )

    if len(time_tokens) != history_sid_end_count:
        raise ValueError(
            f"{row_context}: every historical SID must have one TIME token; "
            f"history_sid_count={history_sid_end_count}, "
            f"time_count={len(time_tokens)}"
        )

    history_events_with_time = HISTORY_EVENT_WITH_TIME_RE.findall(
        input_text
    )

    if len(history_events_with_time) != history_sid_end_count:
        raise ValueError(
            f"{row_context}: historical events are not exactly "
            "CType + SID_BEGIN + A/B/C/D + SID_END + TIME; "
            f"history_sid_count={history_sid_end_count}, "
            f"matched_event_count={len(history_events_with_time)}"
        )
...
def validate_time_tokenization(
    tokenizer,
    time_tokens: Sequence[str],
    validated_time_tokens: Set[str],
    row_context: str,
) -> None:
    """
    Validate each newly observed TIME token once per worker.

    The trained tokenizer must contain every <|time_x|> as one atomic token.
    Otherwise the eval input would no longer match the representation used
    during training.
    """
    unk_token_id = tokenizer.unk_token_id

    for token in dict.fromkeys(time_tokens):
        if token in validated_time_tokens:
            continue
        ...
        if (
            ...
            raise ValueError(
                f"{row_context}: TIME token is unknown to tokenizer: "
                f"{token!r}, token_id={token_id}"
            )

        if encoded != [int(token_id)]:
            raise ValueError(
                f"{row_context}: TIME token must be atomic; "
                f"token={token!r}, convert_id={token_id}, encoded={encoded}"
            )

        validated_time_tokens.add(token)
...
def evaluate_worker(
    ...
    try:
        ...
        # Only newly observed TIME tokens are checked. This keeps strict
        # validation cheap even on a large evaluation parquet.
        validated_time_tokens: Set[str] = set()
        ...
        for row_index, row in tqdm(
            ...
            if STRICT_DATA_VALIDATION:
                time_tokens = validate_sample(
                    input_messages=input_messages,
                    answer_text=answer_text,
                    row_context=row_context,
                )

                validate_time_tokenization(
                    tokenizer=tokenizer,
                    time_tokens=time_tokens,
                    validated_time_tokens=validated_time_tokens,
                    row_context=row_context,
                )

            # Exactly the original chat-template path.
            # Historical CType/SID/TIME tokens remain untouched in input_messages.
            ...
            # Prompt stays in the original residual-SID target-prefix format:
            #   CType + SID_BEGIN
            #
            # Historical TIME tokens have already been preserved inside
            # chat_prompt_ids. No TIME token is appended to the target prefix.
            #
            # The vLLM pooler performs the four-step SID A/B/C/D beam decoding
            # internally on hidden(SID_BEGIN), with interleaved latent thoughts
            # before B/C/D.
